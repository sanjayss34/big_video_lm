# Copyright 2025 big_video_lm authors - Enabled attention mask to be replicated for all attention heads
...
def flash_attention(
  ...
  if ab is not None:
    if ab.shape != (batch_size, 1, q_seq_len, kv_seq_len):
      raise ValueError(
          f"Attention bias shape mismatch: expected ({batch_size=},"
          f" 1, {q_seq_len=}, {kv_seq_len=}), got {ab.shape}"
      )
...
def _flash_attention_impl(
  ...
  def ab_index_map(batch_index, head_index, q_seq_index, kv_seq_index):
    ...
    return (batch_index, 0, next_q_index, next_kv_index)
...
def _flash_attention_bwd_dkv(
  ...
  def ab_index_map(batch_index, head_index, kv_seq_index, q_seq_index):
    return (batch_index, 0, q_seq_index, kv_seq_index)

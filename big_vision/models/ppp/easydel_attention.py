# Copyright 2025 big_video_lm authors - modifications to make ring attention work
...
from big_vision.flash_attention import (
    flash_attention as tpu_flash_attention,
)
from big_vision.ringattention_jax import ring_attention_standard, ring_window_attention_standard
...
from easydel.modules._ring_attention import wise_ring_attention 
...
class AttentionModule:
    ...
    * **Testing and Evaluation:** Includes a `test_attentions` method to systematically evaluate
    ...
    def get_bshd_partition_specs(
        ...
        is_generating = query_sequence_length < 10 # == 1
    ...
    def __call__(
        ...
        with self.mesh:
            ...
            elif self.attn_mechanism == "ring_window":
                return self.ring_attention(
                    query_states=query_states,
                    key_states=key_states,
                    value_states=value_states,
        ...
        *,  # it's Kwarg Only
        ...
        if self.scan_ring_attention and query_states.shape[1] > max(
            ...
            query_sequence_partition = (
                self.partition_axis.generation_query_sequence_axis
                if (query_states.shape[1] < 10)
                else self.partition_axis.query_sequence_axis
            )
            attention_mask = with_sharding_constraint(attention_mask, PartitionSpec(self.partition_axis.batch_axis, None, query_sequence_partition, None))
            if bias is None:
              bias = jnp.zeros((query_states.shape[0], 1, query_states.shape[1], value_states.shape[1]), dtype=query_states.dtype)
            bias += jnp.where(
              attention_mask > 0,
              jnp.zeros_like(bias),
              jnp.full_like(bias, fill_value=-10000.0)
            )
            bias = with_sharding_constraint(bias, PartitionSpec(self.partition_axis.batch_axis, None, query_sequence_partition, None))
            if window_size is not None:
              ring_attention_sharded = shard_map(
                partial(
                  ring_window_attention_standard,
                  segment_ids=None,
                  axis_name=self.axis_name,
                  float32_logits=self.platform == "tpu" and self.force_float32_tpu,
                  scale=1./self.sm_scale,
              ...
              is_query_length_1 = query_states.shape[1] == 1
              ring_attention_sharded = shard_map(
                  partial(
                      ring_attention_standard,
                      segment_ids=None,
    ...
    def flash_attention(
        ...
        *,  # it's Kwarg Only
        ...
        if self.platform == "tpu":
            ...
            print('query sequence length', query_sequence_length, query_states.shape)
            if bias is not None:
            ...
            if self.sm_scale is None:
                self.sm_scale = 1 / math.sqrt(query_states.shape[-1])

            # Pad attention heads and collapse all non-batch sharding into heads dim
            qps_str = str(qps)
            batch_sharding_factor = qps_str.split('(')[1].split(',')[0]
            if batch_sharding_factor.isnumeric():
                batch_sharding_factor = int(batch_sharding_factor)
            else:
                batch_sharding_factor = None
            non_batch_sharding_factors = [int(s) for s in qps_str.split('(')[1].split(')')[0].split(',') if s.isnumeric()]
            new_head_sharding_factor = 1
            for factor in non_batch_sharding_factors:
                new_head_sharding_factor *= factor
            new_qps = jax.sharding.PartitionSpec(batch_sharding_factor, new_head_sharding_factor, None, None)
            new_kps = jax.sharding.PartitionSpec(batch_sharding_factor, new_head_sharding_factor, None, None)

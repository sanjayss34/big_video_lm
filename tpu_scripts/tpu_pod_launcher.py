...
class TPUPodClient:
    ...
    def copy(
        ...
        **kwargs,
        ...
        if excludes is None:
            excludes = []
        hosts = self.list_ips(tpu_name, single_host=single_host)
    ...
    def rsync(
        ...
        **kwargs,
        ...
        strict_host_key_checking_str = "-o StrictHostKeyChecking=no" if not self.strict_host_key_checking else ""
        known_hosts_file_str = f"-o UserKnownHostsFile={self.known_hosts_file}" if self.known_hosts_file is not None else ""
        commands = [f"rsync -vrz -e \"ssh {strict_host_key_checking_str} {known_hosts_file_str} {keypath_str}\" \"{local_path}\" {host}:\"{remote_path}\"" for host in hosts]
        results = run_commands_parallel(commands, **kwargs)
        return {host: result for host, result in zip(hosts, results)}
    ...
    def ssh(
        ...
        **kwargs,
        ...
        if host_index is not None:
            hosts = sorted(hosts)
            hosts = hosts[host_index:host_index+1]
        keypath_str = f"-i {self.key_path}" if self.key_path is not None else ""
        strict_host_key_checking_str = "-o StrictHostKeyChecking=no" if not self.strict_host_key_checking else ""
        known_hosts_file_str = f"-o UserKnownHostsFile={self.known_hosts_file}" if self.known_hosts_file is not None else ""
...
class TPUPodProject:
    ...
    def ssh(
        ...
        **kwargs,
        ...
        command = f"mkdir -p {self.working_dir} && cd {self.working_dir} && {command}"
        return self.client.ssh(self.tpu_name, command, single_host=single_host, host_index=host_index, **kwargs)
    ...
    def scp(
        ...
        **kwargs,
        ...
        return self.client.scp(self.tpu_name, local_path, remote_path, recursive=recursive, single_host=single_host, **kwargs)
    ...
    def rsync(
        ...
        **kwargs,
        ...
        return self.client.rsync(self.tpu_name, local_path, remote_path, single_host=single_host, **kwargs)
    ...
    def copy(
        ...
        **kwargs,
        ...
        for local_path, remote_path in self.copy_dirs:
            result = self.client.copy(self.tpu_name, local_path, remote_path, excludes=excludes, single_host=single_host, **kwargs)
    ...
    def launch(
        ...
        **kwargs,
        ...
        """).strip()
        return self.ssh(command, single_host=single_host, host_index=host_index, **kwargs)
    ...
    def copy_launch(
        ...
        **kwargs,
        ...
        for _ in range(copy_retries):
            ...
            time.sleep(1)
        return self.launch(command, window_name=window_name, single_host=single_host, **kwargs), copy_results
    ...
    def check(
        ...
        **kwargs,
        ...
        results = self.ssh(command, single_host=single_host, **kwargs)
        if not silent:
    ...
    def stop(
        ...
        **kwargs,
        ...
        return self.ssh(command, single_host=single_host, **kwargs)
...
def create_cli(
    ...
    def cli_main(
        ...
        zone: Optional[str]="europe-west4-a",
        ...
        project = create_project(project, zone)
        ...
        commands = dict(
            ...
            launch=lambda command: project.launch(command, verbose=verbose, single_host=single_host, host_index=host_index),
            ssh=lambda command: project.ssh(command, verbose=verbose, single_host=single_host, host_index=host_index),
            scp=lambda l, r: project.scp(l, r, recursive=True, verbose=verbose, single_host=single_host),
            rsync=lambda l, r: project.rsync(l, r, verbose=verbose, single_host=single_host),
            copy=lambda: project.copy(verbose=verbose, single_host=single_host),
        ...
        if mode in commands:
            ...
            elif commands[mode].__name__ == "<lambda>":
                commands[mode](*settings)
            else:
                commands[mode](*settings, single_host=single_host)
        else:
